...
@RestController
@RequestMapping({"/api/v1/words", "/words"})
public class WordController {
  ...
  public WordDetail getWord(
      @AuthenticationPrincipal SearchContext searchContext, @PathVariable long wordId) {
    long userId = searchContext.getUserId();
  ...
  public void createWord(
      @AuthenticationPrincipal SearchContext searchContext, @RequestBody Word newWord) {
    long userId = searchContext.getUserId();
