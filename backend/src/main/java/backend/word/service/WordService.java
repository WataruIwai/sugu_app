...
public class WordService {
  ...
  public WordDetail getWord(long wordId, long userId) {
    Word usersWord =
        wordRepository
            .getWord(wordId, userId)
            .orElseThrow(
                () ->
                    new NotFoundException(
                        "Word not found. wordId=" + wordId + ", userId" + userId));
    List<WordEntry> wordEntries =
        dictionaryRepository.queryWordEntriesData(usersWord.getDictionaryWordId());
  ...
  public void createWord(Word newWord) {
    ...
    DictionaryWord dictionaryWord =
        dictionaryRepository
            .queryWordData(normalizedWord)
            .orElseThrow(
                () ->
                    new IllegalStateException(
                        "Dictionary data should exist when saving a searched word."));
